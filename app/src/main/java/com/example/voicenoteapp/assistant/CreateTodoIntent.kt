...
object MockCreateTodoIntentParser {
    fun parse(transcript: String, today: LocalDate = LocalDate.now()): CreateTodoIntent {
        ...
        return CreateTodoIntent(
            ...
            ambiguities = buildList {
                ...
                if (dueDateIso == null && Regex("\\b(next|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\\b").containsMatchIn(normalized)) {
                    add("Date sounded relative or calendar-based, but this stub parser did not resolve it.")
                }
            }
        )
    }
